...
/**
  ...
  If the extrema of the unimodal function is a segment instead of a point then to get the leftmost or rightmost extremum return value of the function we may consider adding >= instead of > in the "if(  )" part of the code only if necessary.
**/
...
//3. Finding the minimum function (search on integer range)
void ternary_search() {
...
//4. Finding the maximum function (search on integer range)
void ternary_search() {
  int lo = -inf, hi = inf, mid1, mid2;
  while(hi - lo > 4) {
    mid1 = (lo + hi) / 2;
