...
Well written FFT has an error that depends mainly on the magnitude of numbers in the answer, so it works if the resulting polynomial doesn't have numbers bigger than 1015 for doubles and 1018 for long doubles. You can measure the error by finding max of abs(x−llround(x)) over all x in the answer. If it's smaller than 0.2, you're good and rounding will be fine.
**/
