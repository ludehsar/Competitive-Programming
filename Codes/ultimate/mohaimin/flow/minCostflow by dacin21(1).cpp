...
// To get the actual flow collect all the edges which have (f > 0) - this works just like the actual flow finding in normal max flow algorithms;
// Can't find nonMaxflow value upto K. The code is too complex for me to add this function. Maybe its not possible at all and you can only do it with the bellmanFord/Djikstra method.
**/
#include<bits/stdc++.h>
using namespace std;
...
struct mcSFlow{
  ...
  const cost_t INFCOST = numeric_limits<cost_t>::max()/2; ///divide by slightly bigger number if overflow;
  const cost_t INFFLOW = numeric_limits<flow_t>::max()/2; ///divide by slightly bigger number if overflow;
  cost_t epsilon;
  int N, S, T;
...
int main() {
  ...
  for( int test = 1; test <= T; ++test ) {
    ...
    cout << fl.minCostFlow(S, T).first << " " << fl.minCostFlow(S, T).second; ///flow; cost;
  }
}
