...
///For "gaussian elimination modulo version"-
  ...
  4. I have add comments to the code to help with "steps- 2 and 3".
  5. change all doubles to long long as all the calculations are done in long long. omit EPS.
*/
...
int gauss( vector<vector<double>> a, vector<double> &ans ) {
  ...
  for(int col = 0, row = 0; col < m && row < n; ++col) {
    ...
    for(int i = row+1; i < n; ++i)
      if(fabs(a[i][col]) > fabs(a[sel][col])) sel = i; ///For modulo version: if(abs(a[i][col]) > abs(a[sel][col]))
    if(fabs(a[sel][col]) < EPS) { Det = 0.0; continue; } ///For modulo version: if(!a[sel][col])
    ...
    for(int i = 0; i < n; ++i) if (i != row && fabs(a[i][col]) > EPS) { ///For modulo version: if (i != row && a[i][col] > 0)
      double t = a[i][col];
  ...
  for(int i = Rank; i < n; ++i) if(fabs(a[i][m]) > EPS) return 0; ///For modulo version: if(a[i][m])
